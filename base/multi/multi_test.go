...
func TestSchedulers(t *testing.T) {
	...
	m.UpdateEvery(time.Millisecond)
	assertNotUpdated(t, "without any submodule")

	sub1 := m.New()
	sub1.Stream()
	assertUpdated(t, "when new submodule is created")
	assertUpdated(t, "continues to update")
	assertUpdated(t, "if nothing changes")

	m.StopUpdates()
	assertNotUpdated(t, "when updates are stopped")

	sub2 := m.New()
	sub2.Stream()
	assertUpdated(t, "when new submodule is added")
	assertNotUpdated(t, "no repeated update with a second submodule")

	m = NewModuleSet()
	m.OnUpdate(updateFunc)
	sub1 = m.New()
	sub1.Stream()
	assertUpdated(t, "on new submodule")

	sub2 = m.New()
	sub2.Stream()
	assertUpdated(t, "on new submodule")

	assertNotUpdated(t, "without any scheduled updates")
	m.UpdateAfter(time.Millisecond)
	assertUpdated(t, "once when scheduled")
	assertNotUpdated(t, "only once even with two submodules")

	m.UpdateAt(time.Now().Add(time.Second))
	assertNotUpdated(t, "not updated within timeout")
	m.StopUpdates()
	assertNotUpdated(t, "not updated when stopped")
}
